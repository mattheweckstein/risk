...
function DiceDisplay({ result }: { result: AttackResult }) {
  return (
    <div className="p-3 rounded-lg mb-3" style={{ background: 'rgba(0,0,0,0.3)' }}>
      <div className="text-xs font-semibold text-gray-400 mb-2 uppercase">Last Attack</div>
      <div className="flex justify-between mb-2">
        <div>
          ...
          {result.attackerRolls.map((d, i) => (
            <span
              key={i}
              className="inline-flex items-center justify-center w-7 h-7 rounded text-sm font-bold mr-1"
      ...
      <div className="text-xs text-gray-400">
        Attacker lost {result.attackerLosses} | Defender lost {result.defenderLosses}
        {result.conquered && (
          <span className="text-yellow-400 ml-2 font-bold">CONQUERED!</span>
        )}
      </div>
    </div>
  );
...
export default function GameControls({
  ...
  return (
    <div
      className="h-full flex flex-col overflow-y-auto p-4 gap-4"
    ...
    >
        ...
        {phaseOrder.map((phase, i) => (
          <div key={phase} className="flex items-center">
            <div
              ...
              style={{
                background:
                  gameState.phase === phase ? '#e94560' : 'rgba(255,255,255,0.05)',
              }}
            >
              {phaseLabels[phase]}
            </div>
        ...
        <div className="p-3 rounded-lg" style={{ background: 'rgba(233, 69, 96, 0.15)' }}>
          ...
          <div className="text-xs text-gray-400 mt-1">
            {gameState.phase === 'setup'
              ? 'Click a territory you own to place 1 troop'
              : 'Click a territory you own to deploy troops'}
          </div>
        </div>
      )}

      {/* Attack dialog */}
      {gameState.phase === 'attack' && isHumanTurn && selectedTerritory && attackTarget && (
        <div className="p-3 rounded-lg border border-red-500/40" style={{ background: 'rgba(233, 69, 96, 0.1)' }}>
          <div className="text-xs text-red-400 uppercase mb-2 font-bold">Attack</div>
          <div className="text-sm mb-2">
            ...
            <span className="text-gray-400"> ({gameState.territories[attackTarget]?.troops} troops)</span>
          </div>
          <div className="text-xs text-gray-400 mb-2">Select dice count:</div>
      ...
      {/* Fortify dialog */}
      {gameState.phase === 'fortify' && isHumanTurn && fortifySource && fortifyTarget && (
        <div className="p-3 rounded-lg border border-blue-500/40" style={{ background: 'rgba(74, 158, 255, 0.1)' }}>
          <div className="text-xs text-blue-400 uppercase mb-2 font-bold">Fortify</div>
          <div className="text-sm mb-2">
            <span className="text-white font-semibold">
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="text-xs text-gray-400">/ {maxFortifyTroops} max</span>
          </div>
          <button
            onClick={() => {
        ...
        <div className="space-y-1">
          {continentOwners.map(({ continent, owner, bonus }) => (
            <div key={continent} className="flex items-center gap-2 text-xs">
              ...
              <span className="flex-1">{continentDisplayNames[continent]}</span>
      ...
      {gameState.phase === 'ended' && (
        <div className="p-4 rounded-lg text-center" style={{ background: 'rgba(233, 69, 96, 0.2)' }}>
          <div className="text-2xl font-bold text-yellow-400 mb-2">Victory!</div>
          <div className="text-sm text-gray-300">
            {gameState.players.find((p) => p.id === gameState.winner)?.name || 'Unknown'} has conquered the world!
          </div>
        </div>
      )}
    </div>
